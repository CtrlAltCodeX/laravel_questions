    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td class="px-6 py-4">
                    @if($test->photo)
                    <img src="{{ asset('storage/' . $test->photo) }}" alt="Test Image"
                ...
                <td class="px-6 py-4 text-xs">{{ $test->start_date ? $test->start_date->format('d M Y, h:i A') : 'N/A'
                    }}</td>
                <td class="px-6 py-4 text-xs">{{ $test->end_date ? $test->end_date->format('d M Y, h:i A') : 'N/A' }}
                </td>
                <td class="px-6 py-4">
                    <span
                        class="px-2 py-1 rounded text-xs {{ $test->mode == 'auto' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' }}">
                        {{ ucfirst($test->mode) }}
                    </span>
                </td>
                <td class="px-6 py-4 text-center">
                    <span
                        class="px-2 py-1 rounded text-xs {{ $test->status ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        {{ $test->status ? 'Active' : 'Inactive' }}
                    </span>
                </td>
                <td class="px-6 py-4">
                    <div class="flex items-center gap-2">
                        <button type="button" class="text-blue-600 hover:underline edit-test"
                            data-id="{{ $test->id }}">Edit</button>
                        <form action="{{ route('live-tests.destroy', $test->id) }}" method="POST"
...
<div id="modal"
    ...
    <div
        style="background-color: white; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); width: 80%; max-height: 90vh; margin: auto; padding: 24px; position: relative; overflow-y: auto;">
        ...
        <form id="liveTestForm">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1">
                    ...
                    <select id="select_language" name="language_id" required
                ...
                <div class="space-y-1">
                    ...
                    <select id="select_category" name="category_id" required
                        class="w-full border border-gray-300 rounded-lg p-2 text-sm focus:ring-blue-500 focus:border-blue-500">
                ...
                <div class="col-span-2 space-y-1">
                    ...
                    <select id="select_sub_category" name="sub_category_id" required
                        class="w-full border border-gray-300 rounded-lg p-2 text-sm focus:ring-blue-500 focus:border-blue-500">
                    </select>
                </div>
                ...
                <!-- Manual Mode Buttons -->
                <div id="manual_mode_actions"
                    class="col-span-2 hidden bg-gray-50 p-4 rounded-xl border border-gray-200 flex gap-4">
                    <button type="button" id="btn_download_template"
                        class="flex-1 bg-white border-2 border-primary text-primary px-4 py-2.5 rounded-lg text-sm font-bold hover:bg-gray-50 transition-all flex items-center justify-center gap-2 shadow-sm">
                        <i class="fa fa-download"></i> Download Template
                    </button>
                    <button type="button" id="btn_upload_excel"
                        class="flex-1 bg-white border-2 border-primary text-primary px-4 py-2.5 rounded-lg text-sm font-bold hover:bg-gray-50 transition-all flex items-center justify-center gap-2 shadow-sm">
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" name="toppers_star"
                        class="w-full border border-gray-300 rounded-lg p-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                        placeholder="0">
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" name="toppers"
                        class="w-full border border-gray-300 rounded-lg p-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                        placeholder="0">
                </div>
                ...
                <div class="col-span-2 space-y-1">
                    ...
                    <input type="number" name="participant_star"
                        class="w-full border border-gray-300 rounded-lg p-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                        placeholder="0">
                </div>
            </div>
            ...
            <div id="manual_preview_container" class="mt-4 hidden">
                <h3 class="text-sm font-bold text-gray-800 mb-2 flex justify-between items-center">
                    Excel Preview
                    <span class="text-xs font-normal text-gray-500" id="manual_question_count_label">0 questions
                        found</span>
                </h3>
            ...
            <div class="mt-6 pt-4 border-t flex justify-end gap-3 bg-white">
                ...
                <button type="submit" id="saveButton"
                    class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-6 py-2.5 dark:bg-green-600 dark:hover:bg-green-700 focus:outline-none dark:focus:ring-green-800 disabled:opacity-50">Save
                    Live Test</button>
            </div>
        </form>
    </div>
        ...
        $(document).on('click', '.edit-test', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.photo_url) {
                        $('#photo_preview').attr('src', data.photo_url);
                        $('#photo_preview_container').removeClass('hidden');
                    } else {
                        $('#photo_preview').attr('src', '');
                        $('#photo_preview_container').addClass('hidden');
