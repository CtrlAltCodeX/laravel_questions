    ...
    <div class="flex flex-wrap gap-4 items-center w-full md:w-auto justify-end">
        ...
        <div class="flex gap-2">
            ...
            <form id="importForm" action="{{ route('users.import') }}" method="POST" enctype="multipart/form-data"
                class="hidden">
                @csrf
                <input type="file" id="importFile" name="file" accept=".xlsx,.csv"
                    onchange="document.getElementById('importForm').submit()">
            </form>
            <button type="button" onclick="document.getElementById('importFile').click()"
                class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 flex items-center gap-2 text-sm font-semibold transition shadow-sm">
            ...
            <tr
                ...
                <td class="px-6 py-4 text-center">
                    <button class="courseInfoButton text-blue-600 hover:text-blue-800" data-id="{{ $user->id }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mx-auto" fill="currentColor"
                            viewBox="0 0 16 16">
                ...
                {{-- Start Dates --}}
                <td class="px-6 py-4 font-semibold text-gray-600">
                    {{ $user->userCourses->pluck('created_at')->map(fn($date) =>
                    \Carbon\Carbon::parse($date)->format('d-m-Y'))->implode(', ')?:'-' }}
                </td>
                ...
                <td class="px-6 py-4 flex gap-4">
                    ...
                    <form action="{{ route('users.delete', $user->id) }}" method='POST'>
                        @csrf
                        @method('DELETE')
                        <button href="#" class="font-medium text-danger dark:text-danger-500 hover:underline"
...
<div id="modal"
    ...
    <div
        style="background-color: white; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); width: 30%; max-height: 90vh; margin: auto; padding: 24px; position: relative; overflow-y: auto;">
        ...
        <form id="modalForm" method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="coins" id="coins" class="border rounded px-2 py-1 w-full"
            ...
            <button type="submit"
                style="background-color: #2563EB; color: white; font-size: 14px; font-weight: 500; border-radius: 8px; padding: 8px 16px; border: none; cursor: pointer;">Update</button>
        </form>
    </div>
</div>
...
<!-- Course Info Modal -->
<div id="courseModal"
    style="display: none; position: fixed; inset: 0; align-items: center; justify-content: center; z-index: 60; background-color: rgba(0, 0, 0, 0.5);">
    <div
        style="background-color: white; border-radius: 10px; width: 50%; max-height: 90vh; margin: auto; padding: 24px; position: relative; overflow-y: auto;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
