...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg space-y-5">
    <form action="{{ route('topic.index') }}" method="GET">
        <div class="flex gap-x-5">
            <select id="subject_id" name="subject_id" class="bg-gray-50 w-60 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 required-field">
                <option value="">Select Subject</option>
                @foreach($subjects as $item)
                    <option value="{{$item->id}}" {{ isset($subject_id) ? ($subject_id == $item->id  ? 'selected' : '') : '' }}>{{$item->name}}</option>
                @endforeach
            </select>
        ...
        <tbody>
            ...
            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                ...
                <td class="px-6 py-4 flex gap-4">
                    <a href="{{ route('topic.edit', $topic->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
