...
<div class="flex justify-between">
    ...
    <div class="flex gap-2">
        <form action="{{ route('sub-category.index') }}" id='paginationForm'>
            <select class="block px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none cursor-pointer" name="data" id='pagination'>
                <option value="">--Select--</option>
                <option value=1 {{ request()->data == 1 ? 'selected' : ''  }}>All</option>
                <option value=10 {{ request()->data == 10 ? 'selected' : ''  }}>Pagination</option>
            </select>
        </form>
        <a href="{{ route('sub-category.create') }}" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Create</a>
    </div>
</div>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                ...
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                    <img src="{{$sub_category->photo ? '/public/storage/'.$sub_category->photo : '/dummy.jpg'}}" style='width: 50px; height: 50px; border-radius: 50%; object-fit: cover; border:2px solid black;' />
                </th>
                ...
                <td class="px-6 py-4 flex gap-4">
                    <a href="{{ route('sub-category.edit', $sub_category->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
