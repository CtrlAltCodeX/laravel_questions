...
<div class="flex justify-between">
    <h1 class="mb-4 text-2xl font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-2xl dark:text-white">Sub Category</h1>

    <div class="flex justify-end items-center gap-2">
    <a href="{{ route('sub-category.export') }}" class="text-center hover:text-white border border-bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
            Export
        </a>
        ...
        <a href="{{ route('sub-category.sample') }}" class="text-center hover:text-white border border-bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
            Download Sample
        </a>
...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg space-y-5">
    ...
    <tr>
        <th scope="col" class="px-6 py-3">
            <a href="{{ route('sub-category.index', ['sort' => 'id', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}">
                #
                @if ($sortColumn == 'id')
        ...
        <th scope="col" class="px-6 py-3">
            <a href="{{ route('sub-category.index', ['sort' => 'language', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}">
                Language Name
        ...
        <th scope="col" class="px-6 py-3">
            <a href="{{ route('sub-category.index', ['sort' => 'category', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}">
                Category Name
        ...
        <th scope="col" class="px-6 py-3">
            <a href="{{ route('sub-category.index', ['sort' => 'name', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}">
                Sub Category Name
                @if ($sortColumn == 'name')
                        @if ($sortDirection == 'asc')
                            ▲
        ...
        <tbody>
            ...
            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                ...
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                    {{$sub_category->category->language->name}}
                </th>
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
    ...
    data-language-id="{{$sub_category->language_id}}" 
...
<div id="modal" style="display: none; position: fixed; inset: 0; align-items: center; justify-content: center; z-index: 50; background-color: rgba(0, 0, 0, 0.5);">
    <div style="background-color: white; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); width: 50%; margin: auto; padding: 24px; position: relative;">
        ...
        <div class="container">
            ...
            <img class="inline-block h-8 w-8 rounded-full ring-2 ring-white image" src="/dummy.jpg" alt="" id='subCategoryImage' style='width:100px;height:100px;'>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('modalForm').action = '{{ route('sub-category.store') }}';
        ...
        document.getElementById('subCategoryImage').src = '/dummy.jpg'; 
    ...
    editButtons.forEach(button => {
        button.addEventListener('click', function () {
            ...
            document.getElementById('modalForm').action = `/sub-category/${id}`;
            document.getElementById('modalForm').method = 'POST';
        ...
        reader.onload = function() {
            var output = document.getElementById('subCategoryImage');
    ...
    $('#importButton').click(function() {
            ...
            $('#importInput').change(function() {
                var form = $('<form>', {
                    'method': 'POST',
                    'action': '{{ route("sub-category.import") }}',
