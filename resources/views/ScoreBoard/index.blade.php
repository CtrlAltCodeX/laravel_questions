            ...
            <tbody>
                @forelse($UserCourses as $index => $UserCourse)
                    <tr class="odd:bg-white even:bg-gray-50 border-b dark:border-gray-700">
                        ...
                        @php
                            $courseSubs = $UserCourse->course->sub_category_id ?? [];
                            $firstSubCatId = is_array($courseSubs) ? ($courseSubs[0] ?? 0) : (json_decode($courseSubs, true)[0] ?? 0);
                        @endphp
                        <td class="px-6 py-4 text-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="learning-progress-eye cursor-pointer mx-auto"
                                data-user="{{ $UserCourse->user->id }}" data-subcategory="{{ $firstSubCatId }}" width="32"
                                height="32" fill="green" viewBox="0 0 16 16">
                        ...
                        <td class="px-6 py-4 text-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="mock-test-eye cursor-pointer mx-auto"
                                data-user="{{ $UserCourse->user->id  }}" width="32" height="32" fill="blue" viewBox="0 0 16 16">
                                <path d="M8 15A7 7 0 1 0 8 1a7 7 0 0 0 0 14m0 1A8 8 0 1 1 8 0a8 8 0 0 1 0 16" />
                                <path
                                    d="m8.93 6.588-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 .935-.252 1.064-.598l.088-.416c.073-.34.134-.569.288-.569.165 0 .21.207.138.577l-.088.415c-.194.897-.728 1.319-1.532 1.319-1.2 0-1.785-.805-1.532-2.084l.738-3.468c.194-.897.728-1.319 1.532-1.319.545 0 .935.252 1.064.598l.088.416c.073.34.134.569.288.569.165 0 .21-.207.138-.577l-.088-.415c-.194-.897-.728-1.319-1.532-1.319zm-.93-2.588a.905.905 0 1 1 0 1.81.905.905 0 0 1 0-1.81" />
...
@push('scripts')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            function handleEyeClick(selector, options) {
                document.querySelectorAll(selector).forEach(icon => {
                    icon.addEventListener("click", function () {
                        ...
                        let fetchUrl = typeof options.url === 'function' ? options.url(userId, subCatId) : `${options.url}/${userId}`;
            ...
            // Learning Progress
            handleEyeClick(".learning-progress-eye", {
                headings: ["#", "Video Name", "Duration", "Watched Seconds", "Total View Time", "Date/Time"],
                url: (userId, subcatId) => `/api/user/video-progress/${userId}/${subcatId}`,
                title: "Learning Progress Data",
                renderRow: (video, index) => {
                    let watched = parseInt(video.watched_seconds || 0);
                    let total = parseInt(video.video_total_seconds || 0);
                    return `
                        <tr class="border-b">
                            <td class="px-4 py-2">${index + 1}</td>
                            <td class="px-4 py-2">${video.name ?? 'N/A'}</td>
                            <td class="px-4 py-2">${video.duration ?? 'N/A'}</td>
                            <td class="px-4 py-2">${watched} sec</td>
                            <td class="px-4 py-2">${total} sec</td>
                            <td class="px-4 py-2">${new Date(video.created_at).toLocaleString()}</td>
                        </tr>
                    `}
            });
