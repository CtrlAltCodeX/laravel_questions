...
<div class="flex flex-col">
    <div class="flex flex-col">
        <h1
            class="text-2xl font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-2xl dark:text-white flex flex-col justify-between ">
            Question</h1>
    </div>
    <br />
        ...
        <div class="flex justify-end items-center gap-2">
            <form action="{{ route('question.index') }}" method="GET" id='data' class="mb-0 flex gap-2">
                ...
                <div>
                    ...
                    <select name="{{ $moduleKey }}" id='{{ $id }}_select'
                        class="{{ $id }} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 required-field">
                        <option value="">{{$moduleName}}</option>
                        @foreach($module as $item)
                        <option value="{{$item->id}}" {{ $selectedValue==$item->id ? 'selected' : '' }}>{{$item->name}}
                        </option>
                        @endforeach
                    </select>
                ...
                <button id="filter-btn" type="submit"
                    class="text-white text-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 flex items-center me-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Filter</button>
            </form>
        </div>

        <div class="flex justify-end items-center gap-2">
            <a href="/questions.xlsx" download=""
                class="text-center hover:text-white border border-bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Download
                Sample</a>
            ...
            <form action="{{ route('questions.export') }}" class="mb-0">
                ...
                <button type='submit'
                    class="text-center hover:text-white border border-bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Export</button>
            </form>

            <button id="importButton"
                class="text-center hover:text-white border border-bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Import</button>
                ...
                <div class="relative inline-block w-full text-gray-700">
                    <form action="{{ route('question.index') }}" class="mb-0" method="GET" id='page'>
                        ...
                        <div class="flex gap-2">
                            <select
                                class="block px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none cursor-pointer"
                                name="per_page" id='per_page' style="width: 120px;">
                            ...
                            <div class="flex gap-2">
                                <input type="text"
                                    ...
                                    placeholder="Search..." required name="search" value="{{ request()->search }}" />
                    ...
                    <div id="columnSelectDropdown"
                        class="absolute z-10 hidden w-[150px] right-[0px] mt-1 bg-white border border-gray-300 rounded-md shadow-lg">
                        <div class="p-2">
                            <label class="block">
        ...
        <thead>
            <tr id="table-headers">
                ...
                <th scope="col" class="p-2" data-column="id">
                    <a
                        href="{{ route('question.index', array_merge(request()->query(), ['sort' => 'id', 'direction' => $sortColumn == 'id' && $sortDirection == 'asc' ? 'desc' : 'asc'])) }}">
                        ID
                        @if ($sortColumn == 'id')
                ...
                <th scope="col" class="p-2" data-column="question_number">
                    <a
                        href="{{ route('question.index', array_merge(request()->query(), ['sort' => 'question_number', 'direction' => $sortColumn == 'question_number' && $sortDirection == 'asc' ? 'desc' : 'asc'])) }}">
                        QNo.
                ...
                <th scope="col" class="p-2" data-column="language">
                    <a
                        href="{{ route('question.index', array_merge(request()->query(), ['sort' => 'language.name', 'direction' => $sortColumn == 'language.name' && $sortDirection == 'asc' ? 'desc' : 'asc'])) }}">
                        Language
                ...
                <th scope="col" class="p-2" data-column="question">
                    <a
                        href="{{ route('question.index', array_merge(request()->query(), ['sort' => 'question', 'direction' => $sortColumn == 'question' && $sortDirection == 'asc' ? 'desc' : 'asc'])) }}">
                        Question
                ...
                <th scope="col" class="p-2" data-column="optionA">
                    <a
                        href="{{ route('question.index', array_merge(request()->query(), ['sort' => 'option_a', 'direction' => $sortColumn == 'option_a' && $sortDirection == 'asc' ? 'desc' : 'asc'])) }}">
                        Option A
                ...
                <th scope="col" class="p-2" data-column="optionB">
                    <a
                        href="{{ route('question.index', array_merge(request()->query(), ['sort' => 'option_b', 'direction' => $sortColumn == 'option_b' && $sortDirection == 'asc' ? 'desc' : 'asc'])) }}">
                        Option B
                ...
                <th scope="col" class="p-2" data-column="optionC">
                    <a
                        href="{{ route('question.index', array_merge(request()->query(), ['sort' => 'option_c', 'direction' => $sortColumn == 'option_c' && $sortDirection == 'asc' ? 'desc' : 'asc'])) }}">
                        Option C
                ...
                <th scope="col" class="p-2" data-column="optionD">
                    <a
                        href="{{ route('question.index', array_merge(request()->query(), ['sort' => 'option_d', 'direction' => $sortColumn == 'option_d' && $sortDirection == 'asc' ? 'desc' : 'asc'])) }}">
                        Option D
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="p-2" data-column="link">
                    <img src="{{ $question->photo_link ? $question->photo_link : '/dummy.jpg' }}"
                ...
                <td class="p-2" data-column="image">
                    <img src="{{ $question->photo ? 'storage/questions/'. $question->photo : '/dummy.jpg' }}"
                        style="width: 50px; height: 50px; border-radius: 50%; object-fit: cover; border:2px solid black;" />
                </td>
                ...
                <td class="p-2 flex gap-2" data-column="action">
                    <button class="open-edit-modal" data-id="{{ $question->id }}"> <svg
    ...
    ">
        ...
        <form id="question-form" action="{{ route('question.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid gap-5 grid-cols-5">
                ...
                <div class="mb-5">
                    <label for="{{ $id }}"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{$moduleName}}</label>
                    <select id="{{ $id }}" name="module[{{ $moduleKey }}][]"
                        class="{{ $id }} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 required-field">
                        <option value="">{{$moduleName}}</option>
                        @foreach($module as $item)
            ...
            <div class="relative overflow-x-auto sm:rounded-lg">
                ...
                <div class="flex justify-end gap-x-5 mt-5">
                    <button type="button" id="add-row"
                        class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Add
                        New</button>
...
<div id="editModal"
    ...
    ">
        ...
        <form id="questioneditForm" action="{{ route('question.update',  $question->id) }}" method="POST"
